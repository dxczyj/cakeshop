...
<div style='text-align:center;'>
    ...
    <a class='btn btn-info' <c:if test="${page.pageNumber==page.totalPage || page.totalPage==0 }">disabled="disabled"</c:if><c:if test="${page.pageNumber!=page.totalPage}">href="${pageContext.request.contextPath}${param.url }?pageNumber=${page.pageNumber+1}${param.param}"</c:if>>下一页</a>
    <a class='btn btn-info' <c:if test="${page.pageNumber==page.totalPage || page.totalPage==0 }">disabled="disabled"</c:if><c:if test="${page.pageNumber!=page.totalPage}">href="${pageContext.request.contextPath }${param.url }?pageNumber=${page.totalPage}${param.param}"</c:if>>尾页</a>
